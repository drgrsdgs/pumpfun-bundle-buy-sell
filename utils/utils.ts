...
export const saveDataToFile = (newData: DataBuyAmount[], filePath: string = "data.json") => {
  try {
    let existingData: DataBuyAmount[] = [];
...
export function readJson(filename: string = "data.json"): DataBuyAmount[] {
  if (!fs.existsSync(filename)) {
    // If the file does not exist, create an empty array
    fs.writeFileSync(filename, '[]', 'utf-8');
  }
  const data = fs.readFileSync(filename, 'utf-8');
  return JSON.parse(data) as DataBuyAmount[];
}
